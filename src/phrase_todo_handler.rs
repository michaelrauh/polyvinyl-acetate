...
pub fn handle_phrase_todo(todo: NewTodo, holder: &mut Holder) {
    holder.insert_todos("phrase_by_origin", vec![todo.other]);
    holder.insert_todos("phrase_by_hop", vec![todo.other]);
    holder.insert_todos("phrase_by_contents", vec![todo.other]);
}
...
fn get_phrase(holder: &mut Holder, pk: i64) -> Vec<Word> {
    holder.get_phrase(pk)
}
