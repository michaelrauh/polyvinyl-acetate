...
use amiquip::{QueueDeclareOptions, Exchange, Publish, AmqpProperties};
use polyvinyl_acetate::{get_todos, establish_connection, models::Todo, delete_todos};

fn main() {
    loop {
        match apply() {
            Ok(amount) => if amount > 0 { println!("successfully relayed {} messages", amount)},
...
fn apply() -> Result<usize, anyhow::Error> {
    establish_connection().build_transaction()
    .serializable()
        .run(|| { 
            let todos = get_todos()?;
